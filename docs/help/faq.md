...
The wizard can also build UI assets automatically. After onboarding, you typically run the Gateway on port **40104**.
...
- **Tailnet bind**: run `openclaw gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:40104/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 40104:127.0.0.1:40104 user@host` then open `http://127.0.0.1:40104/` and paste the token in Control UI settings.
...
4. **Use the tailnet hostname**
   ...
   - Gateway WS: `ws://your-vps.tailnet-xxxx.ts.net:40104`
...
```
--port > OPENCLAW_GATEWAY_PORT > gateway.port > default 40104
```
...
OpenClaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:40104`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.
...
- If remote, tunnel first: `ssh -N -L 40104:127.0.0.1:40104 user@host` then open `http://127.0.0.1:40104/`.
...
1. Use the WS URL: `ws://<host>:40104` (or `wss://...` if HTTPS).

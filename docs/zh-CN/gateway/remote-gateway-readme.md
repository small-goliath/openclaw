...
│                        Client Machine                          │
│                                                              │
│  OpenClaw.app ──► ws://127.0.0.1:40104 (local port)           │
│                     │                                        │
│                     ▼                                        │
│  SSH Tunnel ────────────────────────────────────────────────│
│                     │                                        │
...
│                         Remote Machine                        │
│                                                              │
│  Gateway WebSocket ──► ws://127.0.0.1:40104 ──►              │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
| 组件                                 | 功能                                  |
| ------------------------------------ | ------------------------------------- |
| `LocalForward 40104 127.0.0.1:40104` | 将本地端口 40104 转发到远程端口 40104 |
| `ssh -N`                             | SSH 不执行远程命令（仅端口转发）      |
| `KeepAlive`                          | 隧道崩溃时自动重启                    |
| `RunAtLoad`                          | 代理加载时启动隧道                    |

OpenClaw.app 连接到你的客户端机器上的 `ws://127.0.0.1:40104`。SSH 隧道将该连接转发到运行 Gateway 网关的远程机器的端口 40104。
